...
Pour trouver tous les points d'ordonnée $2$, on peut s'aider en créant une droite horizontale d'ordonnée $2$, comme ci-dessous.
	\begin{center}
	\includegraphics[page=16, scale=1.5]{figures/fig-affines.pdf}
	\end{center}
...
\dfn{}{
	Un fonction \emphindex{affine} est une fonction $f$ vérifiant pour tout $x\in\R$
		\begin{align}
...
\nt{
	...
	Alors, par définition, nous avons
		\[ \C_f = \bigset{ (x ; f(x) ) \text{ où $x$ parcourt $\R$} }. \]
	Cet ensemble se réécrit également de la façon suivante.
		\[ \C_f = \bigset{ (x ; y ) \text{ où $x$ parcourt $\R$ et où $y = f(x)$} } \]
...
\dfn{équation de droite}{
	Sot $f(x) = ax+b$ une fonction affine sur $\R$. On a
		\[ \C_f = \bigset{ (x ; y ) \text{ où $x$ parcourt $\R$ et où $y =ax+b$} }, \]
	et nomme alors abusivement l'équation \og $y = ax+b$ \fg, l'\emphindex{équation de droite} de $\C_f$.
}{dfn:eq-droite}
...
\lem{coefficient directeur}{
	...
	Alors, pour tout $x\in\R$ réel,
		\[ f(x+1) - f(x) = a. \]
	On lit : « lorsqu'on augmente l'abscisse $x$ de $1$, l'ordonnée $y$ augmente de $a$ ».
}{lem:coeff-dir}
