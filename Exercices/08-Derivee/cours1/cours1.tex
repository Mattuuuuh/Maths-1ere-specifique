...
\subsection{Signe affine et signe de produit}
	...
	\begin{enumerate}
		\item résoudre $ax+b = 0$ pour savoir en quel $x$ elle s'annule ;
		\item reconnaitre si $ax+b$ est croissante, décroissante, ou constante ;
		\item en déduire le signe de $ax+b$ de part et d'autre du zéro à l'aide d'un croquis.
	\end{enumerate}
...
\begin{tabular}{|*{6}{c|}}\hline
	Temps $t$ & 1 & 2 & 1,5 & 1,2 & 0,9
	\\\hline
	Distance parcourue $f(t)$ & a 
	\\\hline
\end{tabular}

La formule de la vitesse est 
	\[ v = \dfrac{\text{différence de distance}}{\text{différence de temps}} = \dfrac{f(t+h)-f(t)}{h}. \]

\begin{enumerate}
	\item
	En utilisant $t=1$ et $t=2$, on obtient une vitesse de
	
	\item
	En utilisant $t=1$ et $t=1,5$, on obtient une vitesse de
	
	\item
	En utilisant $t=1$ et $t=1,2$, on obtient une vitesse de
	
	\item
	En utilisant $t=1$ et $t=0,9$, on obtient une vitesse de
	
\end{enumerate}

La vitesse instantanée de la voiture au temps $t=1$ est donc proche de $10$.
...
\begin{theorem}
	Comme un extremum local de $f$ survient lorsque $f$ change de \hide{variations.}

	Alors un extremum local de $f$ survient lorsque $f'$ change de \hide{signe.}
...
\begin{exemple*}
	Considérons la fonction $f(x) = 2x^3 + 2x + 3$ qu'on étudie pour $-2 \leq x \leq 10$.
	Pour quel(s) $x$ est-ce que $f(x)$ est maximale ? et minimale ?
	\begin{enumerate}
		\item
		La dérivée est de $f$ est 
			\[ f'(x) = .\]
		\item
		On montre que $f'(x) = $ en développant la forme factorisée donnée.
		\item
