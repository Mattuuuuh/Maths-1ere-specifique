...
\fancyhead[C]{\textbf{Fonction dérivée : fonction carré \ifsolutions \\ Solutions \fi}}
\fancyhead[R]{\today}
...
\exe{
	Le but de l'exercice est de calculer, à l'aide de la définition, le nombre dérivé de la fonction carré 
		\[ f(x) = x^2 \]
	en certains points, puis d'en déduire sa fonction dérivée $f'$.
	...
	\begin{enumerate}
		\item On étudie d'abord la fonction $f$.
			\begin{enumerate}[label=\roman*)]
				...
				\item Remplir le tableau de variations de $f$ et de signes de $f'$ figure \ref{fig:var-signe}.
			\end{enumerate}
		...
		\item En déduire la forme algébrique de $f'(x)$ comme fonction de $x$.
