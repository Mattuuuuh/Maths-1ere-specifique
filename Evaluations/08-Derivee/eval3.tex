...
\exe{6,difficulty=1}{
	% f(x) = 4x³-9x²-12x+10
	Nous étudions une fonction $f$ pour des antécédents $x$ situés entre $-2$ et 3.
	...
	\begin{itemize}
		...
		Sa dérivée est
			$ f'(x) = 6(2x+1)(x-2). $
		\item
		L'image de $-2$ est $f(-2) = -34$
		\item
		L'image de $-0,5$ est $f(-0,5) = 13,25$
		\item
		L'image de 2 est $f(2) = -18$
		\item
		L'image de 3 est $f(3) = 1$
	\end{itemize}
	\end{multicols}
	\begin{enumerate}
		...
		\item En déduire le maximum et le minimum de $f(x)$ lorsque $x$ est situé entre $-2$ et 3.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		\tkzTabInit
		 %[lgt=3,espcl=1.5]
	       		{$x$ / 1 , Signe de $2x+1$ / 1, Signe de $x-2$ / 1, Signe de $f'(x)$ / 1, Variation de $f(x)$ / 2}
	       		{$-2$,,,,3}
	\end{tikzpicture}
...
\begin{figure}[h]
	...
	\addplot[domain=-2:3, transparent, thick, samples=100] {4*x^3 - 9*x^2 - 12*x + 10} node[right]{$\C_f$};
	\end{axis}
	\end{tikzpicture}
